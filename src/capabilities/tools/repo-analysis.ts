...
function parseGeminiResponse(geminiText: string): z.infer<typeof repoAnalysisResponseSchema> {
  ...
  try {
    ...
    const mainAnalysisMatch = geminiText.match(/# Main Analysis\s+([\s\S]+?)(?=# Code Insights|# Documentation Insights|$)/i);
    ...
    const recommendationsMatch = geminiText.match(/## Recommendations\s+([\s\S]+?)(?=$)/i);
    if (recommendationsMatch) {
      const recommendationItems = recommendationsMatch[1].match(/- (.+)/g);
      if (recommendationItems) {
        result.documentationInsights.recommendations = recommendationItems.map(item => item.replace(/^- /, '').trim());
      }
    }
  } catch (error) {
    logger.warn('Error parsing Gemini response', { 
      error: error instanceof Error ? error.message : String(error)
    });
    // If parsing fails, return the original text as analysis with empty structured data
  }
